...
function BuildingHelper:LoadSettings()
    BuildingHelper.Settings = LoadKeyValues("scripts/vscripts/kv/building_settings.kv")
    
    BuildingHelper.Settings["TESTING"] = tobool(BuildingHelper.Settings["TESTING"])
    BuildingHelper.Settings["TESTING_DRAWGRID"] = tobool(BuildingHelper.Settings["TESTING_DRAWGRID"])
...
function BuildingHelper:InitGNV()
    ...
    -- print(  'InitGNV: Blocked Map Zones: ' )
    -- PrintTable( blocked_map_zones )
    ...
    for y=boundY1,boundY2 do
        ...
        for x=boundX1,boundX2 do
            ...
            if terrainBlocked or treeBlocked then
    ...
    BuildingHelper:print("Free: "..unblockedCount.." Blocked: "..blockedCount .. " Total: " .. unblockedCount + blockedCount .. " GNV_Length: " .. string.len(gnv_string))
...
function BuildingHelper:BuildCommand(args)
    ...
    -- if BuildingHelper.Settings["TESTING"] then PrintTable(entityList) end
    ...
    if builder == nil then
        builder = PlayerResource:GetSelectedHeroEntity(playerID)
...
function BuildingHelper:CancelCommand(args)
    ...
    -- print( "BuildingHelper:CancelCommand: Right-Click")
...
function BuildingHelper:SetupBuildingTable(abilityName, builderHandle)
    ...
    if IsInToolsMode() then build_time = 1.0 end
...
function BuildingHelper:UpgradeBuilding(building, newName)
    ...
    new_building:RemoveModifierByName("modifier_invulnerable")
    -- PlayerResource:SetOverrideSelectionEntity(playerID, new_building)
...
function BuildingHelper:StartRepair(builder, target)
    ...
    if target:GetClassname() ~= "npc_dota_fort" then
        underConstruction = IsCustomBuilding(target) and target:IsUnderConstruction() -- For RequiresRepair building behaviour
    end
...
function BuildingHelper:BlockPSO(size, location)
    ...
    BuildingHelper:print( " BlockSPO: Size: "..tpstring(size))
...
function BuildingHelper:CellHasGridType(x, y, grid_type)
    if BuildingHelper.GridTypes[grid_type] then
        if BuildingHelper.Grid[y][x] ~= nil then 
...
function BuildingHelper:AddToQueue(builder, location, bQueued)
    ...
    BuildingHelper:print("Building AddToQueue: INIT" )
    -- Check gridnav
...
function BuildingHelper:AddRepairToQueue(builder, building, bQueued)
    ...
    self:print("AddRepairToQueue: bResult: "  .. tostring(bResult) )
...
function BuildingHelper:GetConstructionSize(unit)
    ...
    BuildingHelper:print( "ConstructionSize: " .. unitTable["ConstructionSize"])
